function scr_renderFilter() {
	...
	*/
	...
	var chainListInst = -1;
	with (obj_panelPane) {
		if (currentFunction == functionChainList) chainListInst = self.id;
	}
	...
	with (obj_control) {
		...
		if (!ds_exists(chainList, ds_type_list)) {
			show_debug_message("scr_renderFilter() ERROR: chainList does not exist. chainList: " + string(chainList) + " Exiting...");
			exit;
		}
		...
		for (var j = 0; j < chainListSize; j++) {
			...
			var currentChain = ds_list_find_value(chainList, j);
