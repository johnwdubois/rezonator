	...
	*/
	...
	if(obj_control.combineChains){
		...
		if(selectedChainfocusedChainRow == -1 or currentChainfocusedChainRow  == -1){
			show_debug_message("selected chain row is -1 exiting merge")
		...
		var IDList = ds_grid_get(grid, obj_chain.chainGrid_colWordIDList, selectedChainfocusedChainRow);
		var sizeOfIDList = ds_list_size(IDList);
		
		//show_message(scr_getStringOfList(wIDList) + "      also here is the selected chain ID:   " + string(selectedChainID))
		scr_unFocusAllChains();

		var currentLinkGridRow = ds_grid_value_y(obj_chain.linkGrid, obj_chain.linkGrid_colChainID, 0, obj_chain.linkGrid_colChainID, ds_grid_height(obj_chain.linkGrid), selectedChainID);
		ds_grid_set(obj_chain.linkGrid, obj_chain.linkGrid_colFocus, currentLinkGridRow, true);
		ds_grid_set(grid, obj_chain.chainGrid_colChainState , selectedChainfocusedChainRow, obj_chain.chainStateFocus);

		//delete the clicked on chain 
		...
		//loop through WID List and add links to first chain
		for(var i = 0; i < sizeOfIDList; i++){

			//find next link in chain if stack or rez/track
			if (obj_toolPane.currentTool == obj_toolPane.toolStackBrush) {
