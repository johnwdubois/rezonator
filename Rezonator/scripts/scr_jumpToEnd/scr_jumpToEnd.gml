...
for (var i = 0; i < 2; i++) {
	...
	and currentRow >= 0 and currentRow < ds_grid_height(currentActiveLineGrid) and moves < 30) {
		...
		if (jumpToStart) {
			...
			if (obj_control.searchGridActive) {
				if (lastWordInList >= 0 and lastWordInList < ds_grid_height(obj_control.hitGrid)) {
					var lastWordDisplayCol = ds_grid_get(obj_control.hitGrid, obj_control.hitGrid_colDisplayCol, lastWordInList - 1);
...
if (jumpToStart) {
	...
	if ((furthestDisplayCol) * obj_control.gridSpaceHorizontal + wordLeftMarginDest > window_get_width()) {
