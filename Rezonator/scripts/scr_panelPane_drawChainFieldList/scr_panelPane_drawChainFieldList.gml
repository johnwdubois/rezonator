...
function scr_panelPane_drawChainFieldList(){
	...
	if(chainViewOneToMany){
		var fieldList = global.chainEntryFieldList;
	}
	else{
		var fieldList = global.chainFieldList;
	}
	...
	for (var i = 0; i < fieldListSize+1; i++) {
		...
		if (mouseoverRow && mouseoverWindow) {
			...
			if (mouse_check_button_released(mb_left)) {
				...
				if(chainViewOneToMany){
					...
					with(obj_panelPane){
						functionField_entryFieldSelected = fieldList[| i];
					}
				}
				else{
					...
					with(obj_panelPane){
						functionField_chainFieldSelected = fieldList[| i];
					}
				}
		...
		if (mouseOverDel) {
				...
				if (mouse_check_button_released(mb_left)) {
					...
					if(chainViewOneToMany){
						with (obj_panelPane) functionField_entryFieldSelected = "";
					}
					else{
						with (obj_panelPane) functionField_chainFieldSelected = "";
					}
