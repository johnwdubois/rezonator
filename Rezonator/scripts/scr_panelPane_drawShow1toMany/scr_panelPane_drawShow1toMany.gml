...
function scr_panelPane_drawShow1toMany() {
					...
					if (mouseoverChainRect) {
						...
						if (functionChainList_playShowID != showID) {
							...
							if (mouseOverDel) {
								...
								scr_createTooltip(delButtonX, chainRectY2, scr_get_translation("msg_remove"), TOOLTIP_DIR_UP);
							}
	...
	if (isPlaying || isSelected) {
		...
		if (mouseoverShowPrevButton) {
			scr_createTooltip(mean(showPrevButtonX1, showPrevButtonX2), showPrevButtonY2, scr_get_translation("option_previous"), TOOLTIP_DIR_UP);
		}
		...
		if (isPlaying) {
			showStopButtonText =  "  ■  ";
			if (mouseoverShowStopButton) {scr_createTooltip(mean(showStopButtonX1, showStopButtonX2),mean(showStopButtonY1, showStopButtonY2), scr_get_translation("option_stop"),TOOLTIP_DIR_UP);}
		}
		else {
			if (mouseoverShowStopButton) {scr_createTooltip(mean(showStopButtonX1, showStopButtonX2),showStopButtonY2, scr_get_translation("help_label_play"),TOOLTIP_DIR_UP);}
		}
		...
		if (mouseoverShowNextButton) {scr_createTooltip(mean(showNextButtonX1, showNextButtonX2),showNextButtonY2, scr_get_translation("help_label_next"),TOOLTIP_DIR_UP);}
