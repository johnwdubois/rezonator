...
while (ds_grid_get(currentActiveLineGrid, lineGrid_colPixelY, drawRangeStart) < camera_get_view_y(view_camera[0])) {
	drawRangeStart++;
...
while (ds_grid_get(currentActiveLineGrid, lineGrid_colPixelY, drawRangeEnd) > camera_get_view_y(view_camera[0]) + camera_get_view_height(view_camera[0]) + (obj_control.gridSpaceVertical * 10)) {
	drawRangeEnd--;
