...
function FirebaseREST_Firestore_jsonDecode(json_data)
{
	...
	while(!is_undefined(key)) {
		...
		if (ds_map_is_list(map_fields, key)) {
			value = map_value[| 0];
			show_debug_message("FirebaseREST_Firestore_jsonDecode: key:  "+ string(key) + ", map_value: " + scr_getStringOfList(map_value) + ", value: " + string(value));
		}
		else {
			show_debug_message("FirebaseREST_Firestore_jsonDecode: key:  "+ string(key) + ", map_value: " + json_encode(map_value));
		}
