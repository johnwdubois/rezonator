...
function scr_splitWord(stringList) {
	...
	if(obj_control.fromDropDown){
		scr_newWord(obj_control.rightClickUnitID, obj_control.rightClickWordSeq,ds_list_find_value(listOfWords, 1), obj_control.rightClickWordID);
	}
	else{
		scr_newWord(obj_control.newWordHoverUnitID, obj_control.newWordHoverWordSeq,ds_list_find_value(listOfWords, 1), obj_control.rightClickWordID);
	}
