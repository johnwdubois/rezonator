function scr_fileWindow() {
	...
	if (mouseoverLoadSchema) {
		...
		scr_createTooltip( floor(mean(loadSchemaButtonRectX1, loadSchemaButtonRectX2)), loadSchemaButtonRectY2,scr_get_translation("msg_load-schema"), global.arrowFaceUp);
	...
	if (obj_importMapping.canContinueAll) {
		...
		if (mouseoverSaveSchema) {
			...
			scr_createTooltip( floor(mean(saveSchemaButtonRectX1, saveSchemaButtonRectX2)), saveSchemaButtonRectY2,scr_get_translation("msg_save-schema"), global.arrowFaceUp);
			if (mouse_check_button_pressed(mb_left)) {
