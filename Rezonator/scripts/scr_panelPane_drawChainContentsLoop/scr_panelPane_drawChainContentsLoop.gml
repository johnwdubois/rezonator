...
if (oldRow >= 0 && ds_grid_height(grid) != 0) {
	...
	if (functionChainContents_IDList != undefined) {
		...
		for (var j = currentTopViewRow; j < currentTopViewRow + scrollRange; j++) {	
			...
			if (point_in_rectangle(mouse_x, mouse_y, x + 2, y + textMarginTop + textPlusY - (alignRectSize / 2), x + 2 + alignRectSize, y + textMarginTop + textPlusY + (alignRectSize / 2)) and device_mouse_check_button_released(0, mb_left)
