...
InstallDir "$PROGRAMFILES\Rezonator"
...
!insertmacro MUI_PAGE_INSTFILES
    ...
    !define MUI_FINISHPAGE_RUN "$INSTDIR\Rezonator.exe"
...
Section `${APP_NAME}`
  ...
  WriteRegStr ${INSTDIR_REG_ROOT} "${INSTDIR_REG_KEY}" "DisplayName" "Rezonator"
...
Section "Start Menu Shortcuts"
    ...
    CreateShortcut "$SMPROGRAMS\Rezonator.lnk" "$INSTDIR\Rezonator.exe"
...
Section /o "Desktop shortcut"
  ...
  CreateShortCut "$DESKTOP\Rezonator.lnk" "$INSTDIR\Rezonator.exe" ""
...
Section UnInstall
         ...
         ;uninstall from path, must be repeated for every install logged path individual
         !insertmacro UNINSTALL.LOG_UNINSTALL "$APPDATA\Rezonator"

         ;end uninstall, after uninstall from all logged paths has been performed
         !insertmacro UNINSTALL.LOG_END_UNINSTALL

        Delete "$SMPROGRAMS\Rezonator\Rezonator.lnk"
        Delete "$SMPROGRAMS\Rezonator\uninstall.lnk"
		Delete "$SMPROGRAMS\Rezonator.lnk"
        RmDir "$SMPROGRAMS\Rezonator"
